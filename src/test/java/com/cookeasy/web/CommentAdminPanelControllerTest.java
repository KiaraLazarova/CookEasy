...
public class CommentAdminPanelControllerTest {
    ...
    @Autowired
    public CommentAdminPanelControllerTest(MockMvc mockMvc, UserRepository userRepository, GenderRepository genderRepository, LevelRepository levelRepository, RoleRepository roleRepository) {
    ...
    public void testRetrieveCommentsPageShouldReturnCorrectView() throws Exception {
        this.mockMvc.
                perform(get("/comments")).
                andExpect(status().isOk()).
                andExpect(model().attributeExists("viewModels")).
                andExpect(view().name(GlobalTestConstants.COMMENTS_PAGE_VIEW_NAME));
    }
}