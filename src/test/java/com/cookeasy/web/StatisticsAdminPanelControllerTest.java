...
public class StatisticsAdminPanelControllerTest {
    ...
    @Autowired
    public StatisticsAdminPanelControllerTest(MockMvc mockMvc, UserRepository userRepository, GenderRepository genderRepository, LevelRepository levelRepository, RoleRepository roleRepository) {
    ...
    public void testRetrieveStatisticsPageShouldReturnCorrectView() throws Exception {
        this.mockMvc.
                perform(get("/statistics")).
                andExpect(status().isOk()).
                andExpect(model().attributeExists("viewModel")).
                andExpect(view().name(GlobalTestConstants.STATISTICS_PAGE_VIEW_NAME));
    }
}